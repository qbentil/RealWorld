...
const NavItem: React.FC<NavItemProps> = ({ Icon, name, href }) => {
    ...
    return (
        <p className="flex items-center justify-start w-full">
            <Link href={href} className={`flex items-center justify-start w-full h-full  ${!active ? "text-white hover:text-primary-100 hover:border-r-2 hover:border-primary-100" : "text-primary-50 border-r-2 border-primary-200"}`}>
                <Icon />
    ...
    return (
        <div className="w-full h-full bg-primary-300">
            ...
            <div className="flex items-center justify-center gap-x-2 h-16">
                ...
                <p className='font-bold text-white text-base'>Accede-Feeds</p>
            </div>
            <div className='flex items-start justify-center pl-6 mt-6 gap-y-4 flex-col w-full'>
                {
                    SideNavs.map((nav) => (
                        <NavItem key={nav.name} {...nav} href={`/dashboard${nav.href}`} />
                    ))
                }
            </div>
            ...
            <div className="absolute bottom-0 w-full pl-6">
                <Link href="/signin" className="flex items-center justify-start w-full h-16 text-white hover:text-primary-100">
                    <LogoutCurve />
